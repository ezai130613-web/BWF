...
/** Attendance Management — meeting-wise view, step 1/2 (Select Chapter, Select Meeting). Same picker pattern as /admin/qr-codes and /admin/roster. */
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-neutral-900">Attendance Management</h1>
          <p className="mt-1 max-w-2xl text-sm text-neutral-600">
            Meeting-wise attendance — expected, Present, Absent, and check-in times per member.
          </p>
        </div>
        <div className="flex items-center gap-4">
          <Link href="/api/admin/exports/attendance" className="text-sm text-neutral-600 underline hover:text-neutral-900">
            Export Excel
          </Link>
          <Link href="/admin/attendance/members" className="text-sm text-neutral-600 underline hover:text-neutral-900">
            Member-wise view →
          </Link>
        </div>
      </div>
      ...
      {scope === "ALL" && !chapterId ? (
        <form action="/admin/attendance" method="GET" className="flex flex-col gap-4 rounded-lg border border-neutral-200 bg-white p-6 sm:max-w-sm">
              ...
              <li key={meeting.id} className="flex items-center justify-between py-3">
                ...
                <div className="flex items-center gap-3">
                  <span className="rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-medium text-neutral-500">
                    {meeting._count.attendances} recorded
                  </span>
                  <Link
                    href={`/admin/attendance/${meeting.id}`}
