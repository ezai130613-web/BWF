...
export default async function PaymentsPage({
  ...
  const where: Prisma.PaymentWhereInput = {
    ...(effectiveChapterId ? { member: { chapterId: effectiveChapterId } } : {}),
    ...(params.meetingId ? { meetingId: params.meetingId } : {}),
    ...(params.status ? { status: params.status as PaymentApprovalStatus } : {}),
    ...(params.q ? { member: { ...(effectiveChapterId ? { chapterId: effectiveChapterId } : {}), name: { contains: params.q, mode: "insensitive" } } } : {}),
  ...
  const [payments, summary, meetings] = await Promise.all([
    ...
    db.payment.groupBy({
      by: ["status"],
      where: effectiveChapterId ? { member: { chapterId: effectiveChapterId } } : {},
      _sum: { amountPaidInr: true },
    }),
  ...
  const sumFor = (statuses: string[]) =>
    summary.filter((s) => statuses.includes(s.status)).reduce((total, s) => total + Number(s._sum.amountPaidInr ?? 0), 0);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-neutral-900">Payment Management</h1>
          <p className="mt-1 max-w-2xl text-sm text-neutral-600">
            Meeting payment submissions — independent of attendance. Pending submissions are never
            counted as verified receipts.
          </p>
        </div>
        <div className="flex items-center gap-4">
          <Link href="/api/admin/exports/payments" className="text-sm text-neutral-600 underline hover:text-neutral-900">
            Export Excel
          </Link>
          <Link href="/admin/payments/members" className="text-sm text-neutral-600 underline hover:text-neutral-900">
            Member-wise view →
          </Link>
        </div>
      </div>
      ...
      <form action="/admin/payments" method="GET" className="flex flex-wrap items-end gap-3 rounded-lg border border-neutral-200 bg-white p-4">
        {scope === "ALL" ? (
        ...
        <label className="flex flex-col gap-1 text-xs font-medium text-neutral-600">
          From
          <input type="date" name="from" defaultValue={params.from} className="rounded-md border border-neutral-300 px-2 py-1.5 text-sm" />
        </label>
        <label className="flex flex-col gap-1 text-xs font-medium text-neutral-600">
          To
          <input type="date" name="to" defaultValue={params.to} className="rounded-md border border-neutral-300 px-2 py-1.5 text-sm" />
        </label>
      ...
      <div className="flex flex-col gap-4">
        {payments.map((payment) => (
          <div key={payment.id} className="rounded-lg border border-neutral-200 bg-white p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="text-sm font-medium text-neutral-900">
                  {payment.member.name} · {payment.member.chapter.name}
                </p>
                <p className="text-xs text-neutral-500">
                  {payment.meeting.title} — {payment.meeting.startsAt.toLocaleDateString("en-IN")}
                </p>
              </div>
              <StatusBadge status={payment.status} overlap={payment.overlapsApprovedCoverage} />
            </div>

            <dl className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
              <div>
                <dt className="text-xs text-neutral-500">Amount</dt>
                <dd className="font-medium text-neutral-900">{money.format(Number(payment.amountPaidInr))}</dd>
              </div>
              <div>
                <dt className="text-xs text-neutral-500">Months</dt>
                <dd className="text-neutral-900">{payment.numberOfMonths}</dd>
              </div>
              <div>
                <dt className="text-xs text-neutral-500">Covered</dt>
                <dd className="text-neutral-900">{formatMonthsCovered(payment.monthsCovered)}</dd>
              </div>
              <div>
            ...
            {canApprove ? <PaymentReviewActions paymentId={payment.id} status={payment.status} /> : null}
          </div>
        ))}
        {payments.length === 0 ? (
          <p className="rounded-lg border border-neutral-200 bg-white px-4 py-8 text-center text-sm text-neutral-400">
            No payment submissions match these filters.
          </p>
        ) : null}
      </div>
    </div>
...
function StatusBadge({ status, overlap }: { status: string; overlap: boolean }) {
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      ...
      {overlap ? <span className="text-xs font-medium text-orange-600">⚠ Overlaps approved coverage</span> : null}
    </div>
  );
}
