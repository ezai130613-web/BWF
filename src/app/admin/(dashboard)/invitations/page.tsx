...
/**
 * Meeting Invitation Generator (client spec, 2026-09-18) — step 1/2 (Select
 * Chapter, Select Meeting). Same chapter+meeting picker pattern as
 * /admin/attendance and /admin/visitors-qr; the meeting itself is never
 * created here — only existing meetings from Meeting Management are listed.
 */
export default async function InvitationsPage({
  ...
  const scope = await getChapterScope("invitations:manage");
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <h1 className="text-xl font-semibold text-neutral-900">Meeting Invitation Generator</h1>
        <p className="mt-1 max-w-2xl text-sm text-neutral-600">
          Pick an existing chapter meeting to auto-fill a shareable invitation poster — chapter,
          date, venue, chief guest, and visitor registration QR all come straight from that
          meeting&rsquo;s own record.
        </p>
      </div>
...
async function MeetingPicker({ chapterId, showChapterPicker }: { chapterId: string; showChapterPicker: boolean }) {
  ...
  const meetings = await db.meeting.findMany({
    where: { chapterId },
    include: { invitation: { select: { updatedAt: true } } },
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`rounded-full px-2.5 py-1 text-xs font-medium ${
                      meeting.invitation ? "bg-emerald-50 text-emerald-700" : "bg-neutral-100 text-neutral-500"
                    }`}
                  >
                    {meeting.invitation ? "Invitation saved" : "Not created"}
                  </span>
                  <Link
                    href={`/admin/invitations/${meeting.id}`}
                    className="rounded-md bg-neutral-900 px-3 py-1.5 text-xs font-medium text-white hover:bg-neutral-800"
                  >
                    {meeting.invitation ? "Edit invitation" : "Create invitation"}
                  </Link>
                </div>
              </li>
            ))}
