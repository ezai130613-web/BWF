...
/** Same field set/grouping as the admin EditMemberForm — this is a request, not a publish, so nothing here writes to Member directly (see submitProfileRevision). */
export function RequestProfileEditForm({ member }: { member: Member }) {
  const [state, formAction, pending] = useActionState(submitProfileRevision, initialState);

  if (state?.success) {
    return (
      <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-6 text-sm text-emerald-800">
        Your edit request has been submitted for admin review. You&rsquo;ll see it reflected on your public
  ...
  return (
    <form action={formAction} className="flex flex-col gap-8">
      ...
      <section className="grid gap-4 rounded-lg border border-neutral-200 bg-white p-6 sm:grid-cols-2">
        ...
        <Field label="Video URL (direct file or Google Drive only)" name="videoUrl" type="url" defaultValue={member.videoUrl} />
      </section>
