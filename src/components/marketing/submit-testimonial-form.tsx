...
export function SubmitTestimonialForm({ chapters }: { chapters: { id: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} className="grid gap-4 sm:grid-cols-2">
      ...
      <label className="flex flex-col gap-1.5 text-sm font-medium text-slate-300">
        Company (optional)
        <input name="company" className="rounded-md border border-emerald-600 bg-emerald-900 px-3 py-2 text-sm text-ivory-100 focus:border-gold-500 focus:outline-none" />
      </label>
      <label className="flex flex-col gap-1.5 text-sm font-medium text-slate-300">
        Role (optional)
        <input name="role" className="rounded-md border border-emerald-600 bg-emerald-900 px-3 py-2 text-sm text-ivory-100 focus:border-gold-500 focus:outline-none" />
      </label>
      {chapters.length > 0 ? (
        <label className="flex flex-col gap-1.5 text-sm font-medium text-slate-300">
          Chapter (optional)
      ...
      <label className="flex flex-col gap-1.5 text-sm font-medium text-slate-300 sm:col-span-2">
        ...
        <textarea name="content" rows={4} required className="rounded-md border border-emerald-600 bg-emerald-900 px-3 py-2 text-sm text-ivory-100 focus:border-gold-500 focus:outline-none" />
      </label>
      ...
      <div className="sm:col-span-2">
        <button
          type="submit"
          disabled={pending}
          className="rounded-full bg-gold-500 px-6 py-2.5 text-sm font-medium text-emerald-950 hover:bg-gold-400 disabled:opacity-50"
        >
