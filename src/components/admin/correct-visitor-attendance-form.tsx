...
import { correctVisitorAttendance } from "@/app/admin/(dashboard)/visitors-attendance/[meetingId]/actions";

const initialState: { error?: string; success?: boolean } = {};

/** Spec: corrections require a mandatory reason — collapsed by default so the table stays scannable. */
export function CorrectVisitorAttendanceForm({
  visitorAttendanceId,
  currentStatus,
}: {
  ...
  const [state, formAction, pending] = useActionState(correctVisitorAttendance.bind(null, visitorAttendanceId), initialState);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-2 rounded-md border border-neutral-200 bg-neutral-50 p-3">
      <select name="status" defaultValue={currentStatus} className="rounded-md border border-neutral-300 px-2 py-1 text-xs">
