...
import { reviewVisitorPayment } from "@/app/admin/(dashboard)/visitors-payment/actions";

const initialState: { error?: string; success?: boolean } = {};

/** Approve / Reject (reason required) / Request Clarification — same shape as PaymentReviewActions, gated by payments:approve. */
export function VisitorPaymentReviewActions({ visitorPaymentId, status }: { visitorPaymentId: string; status: string }) {
  const [mode, setMode] = useState<"REJECTED" | "CLARIFICATION_REQUESTED" | null>(null);
  const approveAction = useActionState(reviewVisitorPayment.bind(null, visitorPaymentId, "APPROVED"), initialState);
  const rejectAction = useActionState(reviewVisitorPayment.bind(null, visitorPaymentId, "REJECTED"), initialState);
  const clarifyAction = useActionState(reviewVisitorPayment.bind(null, visitorPaymentId, "CLARIFICATION_REQUESTED"), initialState);
